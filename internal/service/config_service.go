...
type configService struct {
	...
	wafGenRepo    domain.WAFGenerationRepository
...
func (s *configService) GenerateForNode(ctx context.Context, nodeID int) (*domain.GeneratedConfig, error) {
	...
	if wafEnabled, _ := s.settingRepo.Get(ctx, domain.SettingWAFEnabled); wafEnabled.Value == "true" && s.wafGenRepo != nil {
		if wc, err := s.wafGenRepo.LoadForGeneration(ctx); err == nil {
			...
			wafWhitelistMap = buildWhitelistMap(wc.Whitelist)
		}
	}
	...
	return &domain.GeneratedConfig{
		...
		Content:         content,
		HostsMap:        hostsMap,
		WAFBlacklistMap: wafBlacklistMap,
		WAFWhitelistMap: wafWhitelistMap,
		Hash:            hash,
...
// buildBlacklistMap menghasilkan content blacklist.map (satu IP/CIDR per baris).
func buildBlacklistMap(entries []*domain.WAFBlacklist) string {
	var sb strings.Builder
	sb.WriteString("# Generated by HAPM — do not edit manually\n")
	for _, e := range entries {
		sb.WriteString(e.IPAddress)
		sb.WriteByte('\n')
	}
	return sb.String()
}

// buildWhitelistMap menghasilkan content whitelist.map (satu IP/CIDR per baris).
func buildWhitelistMap(entries []*domain.WAFWhitelist) string {
	var sb strings.Builder
	sb.WriteString("# Generated by HAPM — do not edit manually\n")
	for _, e := range entries {
		sb.WriteString(e.IPAddress)
